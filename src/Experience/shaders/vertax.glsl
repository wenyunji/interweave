...
void main(){
    vUv=uv;
    // vNormal=normal;
    vec3 displacedPosition=position+.1*normal*distored_poss(position);
    vec3 eps=vec3(.001,0.,0.);
    vec3 neighbour1=normalize(position+eps.xyy);
    vec3 neighbour2=normalize(position+eps.yxy);
    
    vec3 displacedN1=neighbour1*.1*normal*distored_poss(neighbour1);
    vec3 displacedN2=neighbour2*.1*normal*distored_poss(neighbour2);
